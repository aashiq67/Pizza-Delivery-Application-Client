...
const ForgotPassword = () => {
    ...
    const sendFPemail = async () => {
        const res = await axios.post("http://localhost:5000/admin/forgotpassword", {
            email: email
    ...
    const verifyOTP = async () => {
        const res = await axios.post("http://localhost:5000/admin/verifyotp", {
            OTP: otp
        }).catch(err => console.log(err));
        const data = res.data;
        return data;
    ...
    const setNewPassword = async () => {
        const res = await axios.post("http://localhost:5000/admin/setpassword", {
            password: password
        }).catch(err => console.log(err));
        const data = res.data;
        return data;
    ...
    const handleEmailSubmit = (e) => {
        e.preventDefault();
        sendFPemail().then(alert("OTP Sent.")).then(() => setFP(false))
    }
    ...
    const handlePasswordSubmit = (e) => {
        e.preventDefault();
        setNewPassword().then(() => history("/admin/auth"))
    }
